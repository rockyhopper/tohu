 ...
 * </ol>
 * </li>
 * <li>Question answers - if the client has just answered a question then we care about changes from that value not from the value that was
 * previously on the Question object.</li>
 * </ul>
 * 
...
public class ChangeCollector implements RuleRuntimeEventListener {
	...
	/**
	 * The original values of the objects we have seen. This map will contain nulls to indicate that the "original" of an object was that it
	 * didn't exist. i.e. prior to a create.
	 */
	private transient Map<String, TohuObject> originalObjects;

	/**
	 * Answers provided by the client. This is used to avoid sending back a question to the client if the only change made to the question
	 * was the answer that the client provided.
	 */
	 ...
	 * </p>
	 * <p>
	 * Shallow copies are sufficient since none of our objects contain children. (All lists are stored as comma-delimited strings so they
	 * can be serialized nicely.)
	 * </p>
	 * 
	...
	/**
	 * Fetches the original version of the object for the specified id. If we don't have one then this must be a new object being created so
	 * we add it (as null to indicate that it didn't orginally exist).
	 * 
	 * @param id
	 * @return
	 */
	 ...
	 * @param newObject
	 * @param recentObject
	 *            A recent instance of this object which is used only for removing objects from lists. This is required because it is
	 *            possible for both oldObject and newObject to be null if we are processing a delete right after a create.
	 */
	 ...
	 * </p>
	 * <p>
	 * If the object is a Question then the answer is treated specially because the original value of the answer from our point of view is
	 * the value provided by the client in an Answer fact. If no such fact exists then the value on the question itself is used. Scenarios
	 * are:
	 * </p>
	 * 
	 * <ul>
	 * <li>Question which client has just answered - the new object is different if the answer is not the value provided by the client. i.e.
	 * if the rules have changed it to something else e.g. converting text to upper case.</li>
